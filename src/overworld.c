...
static u8 GetAdjustedInitialTransitionFlags(struct InitialPlayerAvatarState *playerStruct, u16 metatileBehavior, enum MapType mapType)
{
    if (mapType != MAP_TYPE_INDOOR && FlagGet(FLAG_SYS_CRUISE_MODE))
        ...
        {
            /*
            if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_TWIGTON_CITY)) //don't know why this is the case, but Twigton City's tileset wants the player to surf for some reason.
                return PLAYER_AVATAR_FLAG_ON_FOOT;
                */
...
void DoWhiteOutRivalBattle(void)
{
    ...
    HealPlayerParty();
    Overworld_ResetStateAfterWhiteOut();
    SetWarpDestination(MAP_GROUP(MAP_TWIGTON_CITY_PROFESSOR_TWIGS_LAB), MAP_NUM(MAP_TWIGTON_CITY_PROFESSOR_TWIGS_LAB), WARP_ID_NONE, 6, 8);
